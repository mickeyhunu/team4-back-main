...
const service = {
  ...
  async login(params) {
    ...
    try {
      ...
      logger.debug(`(authService.login) ${JSON.stringify(user)}`);
      ...
      logger.error(`(authService.login) ${err.toString()}`);
      return new Promise((resolve, reject) => {
        reject(err);
      });
    }
    ...
    try {
      ...
      logger.debug(`(authService.checkPassword) ${checkPassword}`);
      ...
      logger.error(`(authService.checkPassword) ${err.toString()}`);
      return new Promise((resolve, reject) => {
        reject(err);
      });
    }
